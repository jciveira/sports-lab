...
import { ArrowRight, Loader2, Trophy, Wand2, Plus } from 'lucide-react'
...
import type { DbTeam, TeamCategory, TeamGender } from '../../lib/database.types'
...
export function CreateTournamentPage() {
  ...
  const [cat1, setCat1] = useState<CategorySetup>({ ...EMPTY_CATEGORY, name: 'Categoría 1' })
  const [cat2, setCat2] = useState<CategorySetup>({ ...EMPTY_CATEGORY, name: 'Categoría 2' })
  ...
  // Filter teams by selected category + gender
  ...
  if (!isSupabaseConfigured) {
    return (
      <div className="flex flex-col items-center justify-center min-h-dvh bg-hbl-bg p-6 gap-6">
        ...
        <Link to="/admin/torneos" className="text-hbl-accent text-sm">Volver</Link>
      </div>
    )
  }
...
function InfoStep({ name, date, category, gender, onNameChange, onDateChange, onCategoryChange, onGenderChange, onNext }: {
  name: string
  date: string
  ...
  onCategoryChange: (v: TeamCategory | '') => void
  onGenderChange: (v: TeamGender | '') => void
  onNext: () => void
}) {
  return (
    <div className="flex flex-col gap-4 w-full max-w-sm">
      ...
      <select
        value={category}
        onChange={(e) => onCategoryChange(e.target.value as TeamCategory | '')}
        className="w-full px-4 py-3 rounded-xl bg-hbl-surface border border-hbl-border text-hbl-text focus:outline-none focus:border-hbl-accent"
      >
      ...
      <select
        value={gender}
        onChange={(e) => onGenderChange(e.target.value as TeamGender | '')}
        className="w-full px-4 py-3 rounded-xl bg-hbl-surface border border-hbl-border text-hbl-text focus:outline-none focus:border-hbl-accent"
      >
    ...
    <div className="flex flex-col gap-4 w-full max-w-md">
      <div className="flex items-center justify-center gap-2">
        <input
          ...
          placeholder="Nombre de categoría (ej. Sub-12)"
          maxLength={30}
...
function ConfirmStep({ name, date, category, gender, categories, teams, creating, onBack, onCreate }: {
  name: string
  date: string
  ...
  const totalMatches = categories.length * 12
  const catLabel = [category, gender].filter(Boolean).join(' ')
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-md">
      <div className="flex flex-col gap-3 p-4 rounded-xl bg-hbl-surface border border-hbl-border">
        ...
        {catLabel && (
          <div className="flex items-center justify-between">
            <span className="text-sm text-hbl-text-muted">Categoría</span>
            <span className="font-medium">{catLabel}</span>
          </div>
        )}
        <div className="flex items-center justify-between">
          <span className="text-sm text-hbl-text-muted">Grupos</span>
